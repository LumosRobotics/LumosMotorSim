    ...
    while (sim_time < sim_duration)
    {
        ...
        if (step_count % output_decimation == 0)
        {
            std::cout << std::fixed << std::setprecision(4)
                      ...
                      << "I_rms=" << std::sqrt((motor.current(0) * motor.current(0) + motor.current(1) * motor.current(1) + motor.current(2) * motor.current(2)) / 3.0)
                      << " A" << std::endl;
        }
...
void example_six_step_sensorless()
{
    ...
    while (sim_time < 0.05) // Run for total 50ms
    {
        ...
        case 0:
            floating_phase = 1;
            break; // Phase B floating
        case 1:
            floating_phase = 2;
            break; // Phase C floating
        case 2:
            floating_phase = 0;
            break; // Phase A floating
        case 3:
            floating_phase = 1;
            break; // Phase B floating
        case 4:
...
void example_incremental_encoder()
{
    ...
    while (sim_time < 0.03)
    {
        ...
        if (output_counter % 300 == 0)
        {
            ...
            std::string direction = (count_diff > 0) ? "CW" : (count_diff < 0) ? "CCW"
                                                                               : "STOP";
